...
codex/fix-music-request-button-functionality-lijixw
...
codex/fix-music-request-button-functionality-lijixw
            ${selectedSong && songs.some(song => song.id === selectedSong.id) ? '' : 'disabled'}
...
main
      ...
      <div id="customRequestSection" class="text-center mb-8">
        ...
        <button id="customRequestBtn" onclick="showCustomRequestModal()" class="bg-gray-700 hover:bg-gray-600 px-6 py-3 rounded-xl font-semibold text-base transition shadow-lg border border-gray-500">
          <i class="fas fa-question-circle mr-2"></i>
  ...
  enforceAudienceLayout();
}

function enforceAudienceLayout() {
  const section = document.getElementById('customRequestSection');
  if (!section) return;

  // Keep only the first section if stale scripts injected duplicates
  document.querySelectorAll('#customRequestSection').forEach((el, idx) => {
    if (idx > 0) el.remove();
  });

  // Remove any legacy duplicated buttons outside the canonical section
  document.querySelectorAll('button').forEach((btn) => {
    const txt = (btn.textContent || '').toLowerCase();
    const isOffRepertoire = txt.includes('fora do repertório');
    if (!isOffRepertoire) return;
    if (btn.id === 'customRequestBtn') return;
...
// Show custom request modal (song outside repertoire)
async function showCustomRequestModal() {
  await loadArtist();
  if (isRequestsBlocked()) {
    renderBlockedPage();
    return;
...
async function showRequestModal() {
  ...
  const isSelectedSongAvailable = selectedSong && songs.some(song => song.id === selectedSong.id);
  if (!isSelectedSongAvailable) {
    selectedSong = null;
    updateRequestButtonState();
    showError('Selecione uma música do repertório para pedir.');
    return;
  } else {
    content.innerHTML = `
...
async function submitRequest(event) {
  ...
  const msgEl = document.getElementById('requesterMessage');
  const tipEl = document.getElementById('tipAmount');
  const customTitleEl = document.getElementById('customSongTitle');
  ...
  try {
    // Flow: song outside repertoire (no selected song)
    if (!selectedSong) {
      const customSongTitle = (customTitleEl?.value || '').trim();
      if (!customSongTitle) {
        showError('Informe o nome da música');
        return;
      }

      await axios.post(`/api/artists/${ARTIST_SLUG}/custom-requests`, {
        custom_song_title: customSongTitle,
        requester_name: name || 'Anônimo',
        requester_message: message
      });

      showSuccess('✅ Pergunta enviada! O artista vai confirmar se sabe tocar.');
